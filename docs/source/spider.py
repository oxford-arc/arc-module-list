#
#
# Modified for ARC Module list page
#
# Original from: https://github.com/buildtesters/lmodule
#
# 
# 
import json
...
class Spider:
    ...
    def get_key(self, moduleName, key):
        """Get description of a particular software name. This is can be retrieved by reading ``Description`` key
        in spider output.
        Parameters:
        :param key: name of software
        :type key: str, required
        ...
        # return empty list of key is not found
        if moduleName not in self.get_names():
            return []

        all_keys = []

        for modulefile in self.spider_content[moduleName].keys():
            try:
                all_keys.append(self.spider_content[moduleName][modulefile][key])
            except KeyError:
                all_keys.append("")
